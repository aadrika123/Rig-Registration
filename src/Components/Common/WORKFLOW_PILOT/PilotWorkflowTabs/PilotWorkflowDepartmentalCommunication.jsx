...
function PilotWorkflowDepartmentalCommunication(props) {
  ...
  console.log(
    "dynamic custom form....",
    props?.applicationData?.data?.timelineData?.customFor,
  );
  ...
  const submitFun = (values) => {
    ...
    if (values.remarks == "" && tempFile == null) {
      ...
      setloader(true);
      axios[props?.api?.api_postDepartmentalData?.method](
        props?.api?.api_postDepartmentalData?.url,
        fd,
        ApiHeader2(),
      )
        .then((res) => {
        ...
        .catch((err) => {
          ...
          props?.activateBottomErrorCard(
            true,
            "Some error occured while saving message. Please try again later.",
          );
          setloader(false);
        });
    }
  ...
  useEffect(() => {
    ...
    setloader(true);
    axios[props?.api?.api_getDepartmentalData?.method](
      props?.api?.api_getDepartmentalData?.url,
      {
        applicationId: props?.id,
        customFor: props?.applicationData?.data?.timelineData?.customFor,
      },
      ApiHeader(),
    )
      .then((res) => {
  ...
  return (
    <>
      ...
      {!loader && (
        <div className="container mx-auto ml-0  px-1 py-1 rounded-lg w-full">
          {view ? (
            // Comment list view
            <div
              className={`pb-4 visible text-xs text-gray-600 px-4 pt-2 w-full flex flex-col space-y-2 poppins`}
            >
              ...
              {tempData < 1 && (
                <div className="flex justify-center items-center w-full">
                  <span className="bg-red-100 px-4 py-1.5 rounded-sm shadow-sm">
                    👉Click comment to add comment on this application.👈
                  </span>
                </div>
              )}
              ...
              <div className="w-full flex flex-wrap justify-evenly timeline-container text-xs text-gray-800 poppins">
                {tempData?.map((elem) => (
                  <>
                    ...
                    {elem?.type == "text" &&
                      elem?.customFor ==
                        props?.applicationData?.data?.timelineData
                          ?.customFor && (
                        <div className="timeline-item">
                          <div className="timeline-item-content bg-sky-100 mb-2 mr-2  px-4 py-1 pt-1.5 rounded-md shadow-lg w-[70%]   justify-center">
                            ...
                            <div className="capitalize text-[14px] mb-1 font-semibold">
                              {role}
                            </div>
                            <time>
                              Date :{" "}
                          ...
                          <div className="timeline-item-content bg-sky-100 mb-2 mr-2  px-4 py-1 pt-1.5 rounded-md shadow-lg w-[70%]   justify-center">
                            ...
                            <div className="flex flex-row flex-wrap gap-2">
                              ...
                              <div
                                ...
                                onClick={() => modalAction(elem?.docUrl)}
                              >
      ...
      >
        ...
        >
          <div
            className="absolute top-10 bg-red-200 hover:bg-red-300 right-10 rounded-full p-2 cursor-pointer"
            onClick={closeModal}
          >
            <ImCross />
          </div>
