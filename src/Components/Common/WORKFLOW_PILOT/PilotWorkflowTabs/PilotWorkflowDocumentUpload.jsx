...
function PilotWorkflowDocumentUpload(props) {
  ...
  const [currentOwnerUploadModalData, setcurrentOwnerUploadModalData] =
    useState();
  ...
  const fetchDocumentsToUpload = () => {
    ...
    setloader(true);
    axios[props?.api?.api_uploadDocumentShow?.method](
      props?.api?.api_uploadDocumentShow?.url,
      { applicationId: props?.id },
      ApiHeader(),
    )
      .then((res) => {
        console.log(
          "list of doc to upload at pilotworkflowdocumentupload => ",
          res?.data,
        );
        if (res?.data?.status) {
      ...
      .catch((err) => {
        ...
        props?.activateBottomErrorCard(
          true,
          "Some error occured while fetching document list. Please try again later",
        );
  ...
  const submitData = (docRefName, index, module) => {
    ...
    if (
      docTypeValues[index] === null ||
      docTypeValues[index] === "Select" ||
      docTypeValues[index] === ""
    ) {
      props?.activateBottomErrorCard(
        true,
        `Select document type for ${docRefName}`,
      );
      setcurrentOwnerId(null);
      return;
    }
    ...
    if (
      ...
      (ownerDocRef.current[index] == null ||
        ownerDocRef.current[index] == "" ||
        ownerDocRef.current[index] == undefined)
    ) {
      props?.activateBottomErrorCard(
        true,
        `Select document to upload for ${docRefName}`,
      );
      ownerDocRef.current[index] = null;
      return;
    }
    if (
      ...
      (propertyDocRef.current[index] == null ||
        propertyDocRef.current[index] == "" ||
        propertyDocRef.current[index] == undefined)
    ) {
      props?.activateBottomErrorCard(
        true,
        `Select document to upload for ${docRefName}`,
    ...
    if (!checkSizeValidation(docUpload)) {
      props?.activateBottomErrorCard(
        true,
        `File is too large for ${docRefName}`,
      );
      setdocUpload(null);
      return;
    }
    ...
    fd.append("ulb_id", getLocalStorageItemJsonParsed("ulbId"));
    fd.append("module_id", 15);
    ...
    if (docUpload != null) {
      ...
      axios[props?.api?.api_uploadDocument?.method](
        props?.api?.api_uploadDocument?.url,
        fd,
        ApiHeader2(),
      )
        .then((res) => {
        ...
        .catch((err) => {
          ...
          props?.activateBottomErrorCard(
            true,
            "Some error occured while uploading document. Please try again later",
          );
          setmodal(false);
          setloader(false);
        });
    ...
    <>
                      ...
                      {documentListToUpload?.ownerDocs?.map((owner, index) => (
                        <tr className="border-b border-gray-200 ">
                                ...
                                <div
                                  ...
                                  onClick={() =>
                                    modalFun(
                                      `${owner?.ownerDetails?.uploadedDoc}`,
                                      "jpg",
                                      owner?.ownerDetails?.docId,
                                    )
                                  }
                                >
                                  <img
                                    src={
                                      checkImage(owner?.ownerDetails?.docId)
                                        ? photo
                                        : `${owner?.ownerDetails?.uploadedDoc}`
                                    }
                                    alt=""
                                    className="md:w-[3vw] w-[5vw]"
                                    srcset=""
                                  />
                                </div>
                              )}
                          ...
                          <td className="py-3 px-6 text-left whitespace-nowrap">
                            <div className="flex items-center">
                              <span
                                className={`font-medium ${owner?.ownerDetails?.reqDocCount === owner?.ownerDetails?.uploadedDocCount ? "text-green-400" : "text-red-400"}`}
                              >
                                Uploaded{" "}
                                {nullToNA(
                                  owner?.ownerDetails?.uploadedDocCount,
                                )}{" "}
                                of {nullToNA(owner?.ownerDetails?.reqDocCount)}
                              </span>
                            </div>
                          </td>
                    ...
                    {documentListToUpload?.listDocs?.map((doc, index) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-6">
                          <div className="font-semibold text-sm">
                            <div className="">
                              <select
                                ref={(el) =>
                                  (propertyDocTypeRef.current[index] = el)
                                }
                                ...
                                name="docId"
                                disabled={
                                  (props?.isBtc && doc?.btcStatus) ||
                                  (!props?.isBtc &&
                                    !doc?.btcStatus &&
                                    doc?.uploadedDoc == "") ||
                                  (!props?.isBtc && props?.canUpload) ||
                                  (documentListToUpload?.isCitizen != true &&
                                    documentListToUpload?.isCitizen != "true")
                                    ? false
                                    : true
                                }
                        ...
                        <td className="py-3 px-6 text-center relative">
                          {doc?.uploadedDoc == "" ? (
                            <i className="font-semibold not-italic">NA</i>
                          ) : (
                            <div
                              className="flex items-center justify-center font-semibold text-[26px] cursor-pointer"
                              onClick={() =>
                                modalFun(
                                  `${doc?.uploadedDoc?.docPath}`,
                                  doc?.uploadedDoc?.docPath?.split(".")[
                                    doc?.uploadedDoc?.docPath?.split(".")
                                      ?.length - 1
                                  ],
                                  doc?.uploadedDoc?.uploadedDocId,
                                )
                              }
                            >
                              <div className="flex items-center">
                          ...
                          {doc?.uploadedDoc == "" ? (
                            ...
                            <>
                              <p className="whitespace-no-wrap">
                                {doc?.uploadedDoc?.verifyStatus == 0 && (
                                  <>Pending</>
                                )}
                              </p>
                        ...
                        <td className="py-3 px-6 text-center">
                          {doc?.uploadedDoc?.remarks == "" ||
                          doc?.uploadedDoc?.remarks == null ? (
                            <i className="font-semibold not-italic">NA</i>
                          ) : (
                            doc?.uploadedDoc?.remarks
                          )}
                            ...
                            <>
                              ...
                              (!props?.isBtc && props?.canUpload) ? (
                                <div className="font-semibold text-sm">
                                  ...
                                  {(doc?.uploadedDoc?.verify_status != 1 ||
                                    doc?.uploadedDoc == "") && (
                                    <div className="mt-2">
                                      {!loader && (
                                        <button
      ...
      >
        <div class="relative overflow-x-auto bg-white rounded-lg shadow-xl border-2 border-gray-50 animate__animated animate__zoomIn animate__faster">
          <div
            className="absolute top-5 right-5 bg-red-200 hover:bg-red-300 rounded-full p-2 cursor-pointer"
            ...
            <div className="w-full lg:w-4/6">
              ...
              <div className="bg-white shadow-md rounded my-2">
                <table className="min-w-max w-full table-auto">
                  ...
                  {/* TABLE BODY */}
                  <tbody className="text-gray-600 text-sm font-light bg-white">
                    {currentOwnerUploadModalData?.documents?.map(
                      (doc, index) => (
                        <>
                          <tr className="border-b border-gray-200 ">
                            <td className="py-3 px-6 text-center font-semibold">
                              ...
                              <div className="font-semibold text-sm">
                                <div className="font-semibold text-sm">
                                  <div className="">
                                    <select
                                      ref={(el) =>
                                        (ownerDocTypeRef.current[index] = el)
                                      }
                                      className="form-control block w-full px-3 py-1 text-base md:text-xs font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none cursor-pointer shadow-md w-36"
                                      name="docId"
                                      disabled={
                                        ...
                                        (!props?.isBtc &&
                                          ...
                                          doc?.uploadedDoc == "") ||
                                        (!props?.isBtc && props?.canUpload) ||
                                        (documentListToUpload?.isCitizen !=
                                          true &&
                                          documentListToUpload?.isCitizen !=
                                            "true")
                                          ? false
                                    ...
                                    <div className="font-semibold text-sm">
                                      <div className="">
                                        <input
                                          ...
                                          type="file"
                                          onChange={(event) =>
                                            handleChange(event, "owner", index)
                                          }
      ...
      >
        <div class=" rounded-lg shadow-xl border-2 border-gray-200 bg-white md:ml-32 p-4 animate__animated animate__zoomIn animate__faster w-full md:w-[60vw] h-[80vh]">
          <div
            className="absolute top-2 font-normal bg-red-200 hover:bg-red-300 right-2 rounded-full p-2 cursor-pointer"
