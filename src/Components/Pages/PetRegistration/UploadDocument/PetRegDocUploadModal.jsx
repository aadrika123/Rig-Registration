...
function PetRegDocUploadModal(props) {
  ...
  const docUpload = (e) => {
    ...
    formData.append("docCode", docType);

    formData.append("ulb_id", getLocalStorageItemJsonParsed("ulbId"));
        ...
        <form encType="multipart/form" onSubmit={docUpload}>
          <div className="bg-indigo-200 shadow-2xl border border-sky-200 p-2 rounded-md">
            <div className="my-2 mx-3">
              <div
                onClick={closeModal}
                className="float-right cursor-pointer hover:bg-gray-300 rounded-sm p-1"
              >
                {" "}
                <GrClose />
              </div>
              <h1 className="font-semibold md:text-xl text-sm text-gray-700">
                Upload Document
              ...
              <table className="min-w-full leading-normal border">
                <thead className="bg-sky-100">
                  <tr className="font-semibold ">
                    <th
                      scope="col"
                      className="px-5 py-2 border-b border-gray-200 text-gray-800  text-left text-sm uppercase"
                    >
                      #
                    </th>
                    <th
                      ...
                      <td className="px-5 py-2 border-b border-gray-200 bg-white text-sm">
                        <p className="text-gray-900 whitespace-no-wrap">
                          ...
                          <select
                            ...
                            className=" outline-blue-600 border border-gray-400 w-40"
                          >
                            <option>Select Document</option>
                            {data?.masters?.map((item) => (
                              <option value={item.documentCode}>
                                {item.docVal}
                              </option>
                            ))}
                          </select>
                      ...
                      <td className="px-5 border-b border-gray-200 bg-white text-sm">
                        <span className="relative inline-block ">
                          ...
                          <span className="relative ">
                            ...
                            />
                            <label
                              className={` ${isUploading ? "cursor-wait hover:rounded-none" : "cursor-pointer rounded-sm hover:rounded-md hover:shadow-2xl hover:bg-blue-600 hover:ring-1 hover:ring-blue-800"} bg-blue-500  shadow-lg flex pl-4 pr-5 py-2  text-white font-medium`}
                              htmlFor="contained-button-file"
                            >
                              <FiUpload
                                size={16}
                                className="mt-0 ml-0 mr-2 font-medium"
                              />
                              Browse File
                            </label>
                          </span>
                      ...
                      <td className="px-5 py-2 border-b border-gray-200 bg-white text-sm">
                        {isUploading ? (
                          ...
                          <button
                            disabled={isUploading}
                            type="submit"
                            className="border disabled:bg-green-300 disabled:text-gray-500 bg-green-600 rounded-md text-white px-4 py-2"
                          >
                            Submit
                          </button>
                        )}
