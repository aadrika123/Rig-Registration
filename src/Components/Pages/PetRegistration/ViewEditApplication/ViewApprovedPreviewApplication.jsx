...
const ViewApprovedPreviewApplication = () => {
    ...
    const { api_PetPreviewViewApplication, header } = PetRegAPIList();
    
    // Effect hook to fetch application data when the component mounts
    useEffect(() => {
        setLoader(true)
        AxiosInterceptors.post(api_PetPreviewViewApplication, { "registrationId": id }, header)
            .then((res) => {
                setLoader(false)
                if (res.data.status) {
...
export default ViewApprovedPreviewApplication