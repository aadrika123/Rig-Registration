...
const ViewPetApplication = () => {
  ...
  const { api_PetRegViewApplication, header, api_RigUploadedDoc, docReUpload } =
    PetRegAPIList();
    ...
    <>
      ...
      <div className="col-span-2 space-y-2">
        {applicationFullData?.payment_status == 0 &&
          applicationFullData?.applicantsStatus == 1 && (
            <p className="text-xl font-serif text-orange-500">
              Your application is Approved and Payment is Pending.
            </p>
          )}
        {applicationFullData?.payment_status == 0 &&
        applicationFullData?.applicantsStatus != 1 ? (
          <p className="text-xl font-serif">
            ...
            <span className="font-semibold text-orange-700">
              {applicationFullData?.roleName}
            </span>
          </p>
        ) : (
          <>
            {applicationFullData?.payment_status == 2 ? (
              <p className="text-xl font-serif">
                {" "}
                Your application payment is under :{" "}
        ...
        <p className="text-xl font-serif">
          ...
          <span className="font-semibold text-blue-700">
            {applicationFullData?.application_no}
          </span>
        </p>
        <p className="text-xl font-serif">
          Application Remarks :{" "}
          <span className="font-semibold text-green-700">
            {applicationFullData?.remarks}
          </span>
        </p>
        ...
        {applicationFullData?.payment_status == 1 &&
          applicationFullData?.registrationStatus == 2 &&
          applicationFullData?.transactionDetails?.verify_status == 1 && (
        ...
        {/* Rig edit  details */}
        {applicationFullData?.payment_status == 0 &&
        applicationFullData?.applicantsStatus == "" &&
        applicationFullData?.application_type == "New_Apply" ? (
          <div className="space-x-5 flex justify-end">
            <button
              className="bg-sky-600 hover:bg-sky-500 text-white px-5 py-1 rounded shadow flex justify-end "
              onClick={openDialogModal2}
            >
              Edit Application
            </button>
        ...
        {applicationFullData?.isRenewal && (
          <div className="space-x-5 text-right px-4">
            <button
              className={`font-semibold md:text-base text-xs bg-indigo-500 text-white border border-indigo-500  px-4 py-1 shadow-lg hover:scale-105 rounded-sm`}
              onClick={() => navigate(`/rig-renewal/${id}`)}
            >
              {" "}
              Apply Renewal
            </button>
      ...
      <div className="grid grid-cols-12 mb-20">
        ...
        <div className="rounded-md col-span-12">
          ...
          {applicationFullData?.current_role_name && (
            <p className="text-center text-green-600 font-semibold">
              {" "}
              Your Application Submitted. Currently at{" "}
              {applicationFullData?.current_role_name}
            </p>
          )}

          {somethingWentWrong == true ? (
            <p className="m-auto mt-10 font-semibold border border-gray-300 text-red-700 bg-red-100 px-5 py-2 rounded shadow w-fit">
              Failed to Get Data
            </p>
          ) : (
            <div className="overflow-y-auto">
              ...
              {loader ? (
                ...
                applicationFullData?.doc_upload_status && (
                  <div className="bg-white shadow-xl p-4 border border-gray-200 my-5">
                    <h1 className="px-1 font-semibold font-serif text-xs text-[#37517e]">
                      <img
                        ...
                        <>
                          {docDetails?.map((items, i) => (
                            <tr className="bg-white shadow-lg border-b border-gray-200">
                              ...
                              <td className="px-2 py-2 text-sm text-left text-[#37517e]">
                                {items?.doc_code ? items?.doc_code : "N/A"}
                              </td>
                              ...
                              <td className="px-2 py-2 text-sm text-left text-[#37517e]">
                                ...
                                >
                                  {items?.doc_path?.split(".").pop() ==
                                  "pdf" ? (
                                    <img
                                      className="h-10 w-10 border rounded shadow-md"
                                      src={pdfImage}
                                    />
                                  ) : items?.doc_path ? (
                                    <img
                              ...
                              <td className="px-2 py-2 text-sm text-left text-[#37517e]">
                                {items?.remarks ? items?.remarks : "N/A"}
                              </td>
                              <td className=" text-xs text-left text-[#37517e]">
                                ...
                                {items?.verify_status == 2 && (
                                  <p>
                                    <button
                                      className=" text-xs text-left  bg-sky-400 px-2 py-1 text-white rounded-sm"
              ...
              <div className="bg-white shadow-xl p-4 border border-gray-200">
                <h1 className="px-1 font-semibold font-serif text-xs mt-2 text-[#37517e]">
                  <img
                    src="https://cdn-icons-png.flaticon.com/512/8948/8948774.png"
                    alt="Upload"
                    className="w-5 inline text-[#37517e]"
                  />{" "}
                  Payment Details
                </h1>
                {applicationFullData?.transactionDetails?.payment_mode ==
                  "CHEQUE" &&
                  applicationFullData?.transactionDetails?.verify_status ==
                    2 && (
                  ...
                  <div className="text-center text-indigo-600">
                    <div>
                      ...
                      <div className="flex justify-center">
                        <button
                          onClick={() => navigate(`/rig-payment-offline/${id}`)}
                          className="px-2 py-1 bg-indigo-600 text-sm text-white"
                        >
                          Pay Now
                        </button>
                      </div>
                  ...
                  <div className="mt-2">
                    ...
                    {applicationFullData?.transactionDetails && (
                      <div className="flex space-x-10 pl-4 my-2 border-y-gray-200">
                        ...
                        <div className="flex-1 text-xs">
                          <div className="font-semibold text-sm whitespace-nowrap text-[#37517e]">
                            {applicationFullData?.transactionDetails?.tran_no ||
                              "N/A"}
                          </div>
                        </div>
                        <div className="flex-1 text-xs">
                          <div className="font-bold text-sm text-[#37517e]">
                            {applicationFullData?.transactionDetails?.amount ||
                              "N/A"}
                          </div>
                        </div>
                        <div className="flex-1 text-xs">
      ...
      <dialog ref={editPetApplicationRef2}>
        <div className="float-right p-2">
          <p
            className="bg-red-600 rounded-full px-3 py-1 cursor-pointer text-white font-semibold"
            onClick={closeDialogModal2}
          >
            x
          </p>
        </div>
        <EditPetDetailsForm
          editPetApplicationRef2={editPetApplicationRef2}
          applicationFullData={editPetData}
          applicationFullData1={applicationFullData}
              ...
              applicationFullData?.doc_upload_status && (
                <form onSubmit={formik.handleSubmit} onChange={handleChange}>
                  <div className="  p-4   my-5">
                    ...
                    <h1 className="px-1 font-semibold font-serif text-lg text-center text-[#37517e]">
                      <img
                        src="https://cdn-icons-png.flaticon.com/512/2029/2029957.png"
                        alt="pin"
                        className="w-5 inline text-[#37517e] ml-2"
                      />{" "}
                      &nbsp; RE-UPLOAD Document{" "}
                    </h1>
