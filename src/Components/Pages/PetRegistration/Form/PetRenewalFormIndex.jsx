    ...
    <>
      <form onSubmit={formik.handleSubmit} onChange={handleChange}>
        <div className="overflow-y-auto">
          <h1 className="px-4 font-semibold text-lg text-center">
            {" "}
            Application no -
            <span className="text-blue-600"> {masterData?.application_no}</span>
          </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 xl:grid-cols-3  bg-white shadow-md rounded-md py-2">
            ...
            <div className="m-3">
              <label className={style?.label} htmlFor="categoryApplication">
                Category of Application
              </label>
              <input
                disabled
                value={
                  masterData?.application_type == "New_Apply" ? "Renewal" : ""
                }
                type="text"
                name="holdingNo"
                className={style?.textFiled}
              />
              <p className="text-red-500 text-xs">
                {formik.touched.categoryApplication &&
                formik.errors.categoryApplication
                  ? formik.errors.categoryApplication
                  : null}
              </p>
            </div>
          ...
          <div className="bg-white shadow-md rounded-md py-2">
            <div className="grid grid-cols-1 md:grid-cols-3 ">
              <div className="m-3">
                ...
                <input
                  disabled
                  value={masterData?.applicant_name}
                  className={style?.textFiled}
                />
                <p className="text-red-500 text-xs">
      ...
      <dialog ref={dialogRef} className="">
        <div className="space-y-2 mb-5">
          <p className="text-center bg-yellow-300 p-2 font-semibold ">
            {" "}
          ...
          <div className="flex justify-center gap-x-5">
            <button
              onClick={() => navigate(`/search-pet-registration`)}
              className="border border-indigo-600 px-2 py-1 rounded shadow hover:bg-indigo-500 hover:text-white"
            >
              Dashboard
            </button>
            <button
              onClick={() =>
                navigate(`/pet-payment-offline/${userDetails?.id}`)
              }
              className="bg-indigo-500 px-2 py-1 rounded shadow text-white hover:bg-indigo-700"
            >
              Make Payment
            </button>
          </div>
