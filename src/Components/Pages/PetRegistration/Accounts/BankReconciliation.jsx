...
function BankReconciliation () {
    ...
    const fetchFilterData = (parameters) => {
      ...
      AxiosInterceptors
        .post(api_collectionReport, payloadData, header)
        .then(
    ...
    const getDetailsFun = (id, mId) => {
      ...
      AxiosInterceptors.post(getReconcileById, body, ApiHeader())
      .then((res) => {
    ...
    const COLUMNS = [
      ...
      },
      {
        ...
        {cell?.row?.original?.status == '1' && <span className="text-green-500 font-semibold">
          ...
          {cell?.row?.original?.status == '0' && <span className="text-blue-500 font-semibold">
            Pending
          </span>}
          {cell?.row?.original?.status == '2' && <span className="text-red-500 font-semibold">
            Bounce
          </span>}
        </>
      },
